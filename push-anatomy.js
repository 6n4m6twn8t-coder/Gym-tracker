// Static Push muscle-focus cards. One clean image per exercise; no sprite and no SVG anatomy overlay.
(()=>{
const FILES={
...
style.textContent=`
.muscleFocus{margin:12px 0 2px;height:190px;border:1px solid #272c36;border-radius:14px;overflow:hidden;background:#090b0f;display:grid;place-items:center;box-shadow:0 1px 0 rgba(255,255,255,.025) inset}
...
Promise.all(Object.entries(FILES).map(async([name,path])=>{
  try{
    const r=await fetch(`${path}?v=2`,{cache:'no-store'});
